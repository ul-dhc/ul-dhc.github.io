const translations = {
  lv: {
    ...
    heroItems: [
      'izdarīt labu darbu – veltīt dažas minūtes latviešu valodas nākotnei',
      'spēlējot noskaidrot, cik labi pazīstat LU',
      'uzrakstīt novēlējumu, kas dosies līdzi LU nākamajā gadā',
    ],
    programmeLink: 'Klātienes pasākumu programma',
    heroButton: 'Izvēlies, kā svinēt tiešsaistē',
    choiceTitle: 'Mēs aicinām svinēt LU-107',
    choiceKickers: ['Piedaloties', 'Spēlējot', 'Novēlot'],
    choiceTitles: ['Izdari labu darbu!', 'Cik labi Tu pazīsti LU?', 'Uzraksti novēlējumu!'],
    choiceTexts: [
      'Velti dažas minūtes latviešu valodas nākotnei – ierunā sveicienu Latvijas Universitātei vai atbildi uz jautājumu par LU.',
    ...
    voiceTexts: [
      ...
      'Novērtē, vai ieraksti ir pareizi marķēti, un atzīmē tos, kas jāpārbauda vēlreiz.',
    ],
    ...
    gamePreviewText: '23 uzdevumi piecās kārtās · aptuveni 5 minūtes.',
    ...
    wishLabel: 'Novēlējums',
    wishPlaceholder: 'Uzraksti savu novēlējumu…',
    nameLabel: 'Vārds <small>(nav obligāts)</small>',
  ...
  en: {
    ...
    programmeLink: 'In-person events programme',
    heroButton: 'Choose how to celebrate online',
    ...
    voiceTexts: [
      ...
      'Help assess recordings made by others. Reviewing them is equally important.',
      'Check whether recordings are labelled correctly and flag those that need another review.',
    ],
    ...
    gamePreviewText: '23 tasks in five rounds · approximately 5 minutes.',
    ...
    nameLabel: 'Name <small>(optional)</small>',
...
const setManyText = (selector, values) => {
  document.querySelectorAll(selector).forEach((element, index) => {
    if (values[index] !== undefined && element.textContent !== values[index]) element.textContent = values[index];
  });
};
...
function ensureLanguageSwitcher() {
  ...
  switcher.innerHTML = '<a href="#lv" data-language="lv" lang="lv">LV</a><a href="#en" data-language="en" lang="en">EN</a>';
...
function ensureWishWallLayout() {
  const section = document.querySelector('.wish-section');
  const heading = section?.querySelector('.wish-heading');
  const form = heading?.querySelector('.wish-form');
  if (!section || !heading || !form) return;
  if (!section.hasAttribute('data-form-open')) section.setAttribute('data-form-open', 'false');
  let toggle = heading.querySelector('.wish-form-toggle');
  if (!toggle) {
    toggle = document.createElement('button');
    toggle.type = 'button';
    toggle.className = 'wish-form-toggle';
    toggle.setAttribute('aria-expanded', 'false');
    toggle.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-plus" aria-hidden="true"><path d="M5 12h14"></path><path d="M12 5v14"></path></svg>';
    heading.insertBefore(toggle, form);
  }
...
function translatePage(language) {
  ...
  let programmeLink = document.querySelector('.header-programme-link');
  if (!programmeLink) {
    programmeLink = document.createElement('a');
    programmeLink.className = 'header-programme-link';
    ...
    const header = document.querySelector('.site-header');
    header?.insertBefore(programmeLink, header.querySelector('.language-switcher'));
  }
  ...
  setText('.header-programme-link span', t.programmeLink);
  programmeLink?.setAttribute('aria-label', t.programmeLink);
  programmeLink?.setAttribute('title', t.programmeLink);
  ...
  const nameInput = document.querySelector('.wish-form input:not([type="checkbox"])');
  if (textarea) textarea.placeholder = t.wishPlaceholder;
...
document.addEventListener('click', async (event) => {
  ...
  if (languageLink) {
    event.preventDefault();
    history.replaceState(null, '', `#${languageLink.dataset.language}`);
...
window.addEventListener('hashchange', applySelectedLanguage);
window.addEventListener('load', applySelectedLanguage, { once: true });
setTimeout(applySelectedLanguage, 1100);
applySelectedLanguage();
